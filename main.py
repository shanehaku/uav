...
def main():
    ...
    while True:
        ...
        if not task1_finished:
            ...
            if task2_finished:
                up_down = -task2.distance[1]-0.2*task2.distance[2]+30
                if task1.direct=="Right":
                    ...
                    drone.move_forward(int(task2.distance[1])+50)
                    #回正
                    if abs(int(task2.distance[0]))>20:
        ...
        #             break
            ...
            # TODO 3 should be change into scan result of yolo
            task3_finished = task3.run(tag_list, frame ,3)
            if task3_finished:
                if task3.distance[0]>=0:
                    if abs(int(task3.distance[0]))>20:
                        drone.move_right(abs(int(task3.distance[0])))
                    drone.move_forward(int(task3.distance[1])-10)
                else:
                    if abs(int(task3.distance[0]))>20:
